...
ユーザー指定の必須フォーマットおよび各週数段階に応じた表示を行う。
- **メイン表示（カウントダウン中）**:
  - **「W週D日 {目標週}週目まであとh時間(d日)」**
    - 例: `9週3日 12週目まであと432時間(18日)`
    - 例: `11週6日 12週目まであと3時間(1日)`
- **12週達成時の表示**:
  - 12週（84日）に到達した場合、「12週目を迎えました！本当にお疲れ様でした💐体調はいかがですか？」という祝福メッセージと、15週目標への切り替えボタンを表示する。
- **15週経過後の表示（安定期以降）**:
  - **15週を過ぎた場合（15週0日以降 / 16週以降）は、カウントダウン表示を行わず「現在の週数を表示するのみ」とする**。
    - 例: `16週2日`
    - 「つわりの目安期間（15週）を乗り越えました！体調はいかがですか？💐」というねぎらいメッセージとともに、シンプルに現在の週数を穏やかに確認できる画面となる。
...
2. **メインカウントダウン画面（〜15週）**:
   ...
   - メインカウントダウンフォーマット（`9週3日 12週目まであと432時間(18日)`）。
   - 目標切り替えタブ（12週／15週）。
   - プログレスバー & 寄り添いメッセージ。
3. **15週達成・週数表示画面（15週超）**:
