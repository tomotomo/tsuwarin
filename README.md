...
### 4. ⏳ 重複ゼロ・1つの美しい統合文字組み
「特大数字」と「主文プレート」で同じ残り時間を2度並べる認知負荷を撤廃。  
`9週3日` → `12週目まで あと` → `424 時間` → `(約 18日)` のように、1本の自然な視線誘導で美しく統合されています。

### 5. 🌱 絶望メーター（プログレスバー）の廃止
ピーク期（7〜8週）に「まだこれだけしか進んでいない」という絶望感を与える進捗バーを完全排除。「前回アクセスから耐えて前進した時間（+○時間）」だけを肯定的なピルバッジとしてそっと伝えます。

### 6. 💐 15週を乗り越えた後の穏やかな佇まい
出口の目安（15週）を超過した後は、カウントダウンタイマーを停止。現在の週数（例: `現在 16週2日`）とねぎらいの言葉だけを静かに確認できる画面に移行します。
...
### iPhone (Safari)
1. 公開URL（[https://tomotomo.github.io/tsuwarin/](https://tomotomo.github.io/tsuwarin/)）にアクセス
